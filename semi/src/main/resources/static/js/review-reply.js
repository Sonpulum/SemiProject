...
$(function(){
	...
	$(".reply-insert-btn").click(function(){
		var content = $("[name=reviewReplyContent]").val();//입력값 불러오기
		...
		$.ajax({
			...
			success:function(response){
				loadList();//목록 불러오기
				$("[name=reviewReplyContent]").val("");//입력창 청소
			},
			error:function(){
				alert("통신 오류 발생\n잠시 후 다시 시도하세요");
			}
		...
		$.ajax({
			...
			success:function(response){//response == List<ReplyDto>
				...
				for(var i=0; i < response.length; i++) {
					...
					// -> memberId==response[i].replyWriter
					if(reviewWriter == response[i].reviewReplyWriter){
						var author = $("<span>").addClass("author ms-10").text("작성자");
						$(html).find(".reviewReplyWriter").append(author);
					}
					...
					if(memberId == response[i].reviewReplyWriter || memberLevel == "관리자"){
						...
						$(html).find(".reviewReplyWriter").append(editButton)
													.append(deleteButton);
					}
	...
	function editReply(){
		...
		var textarea = $("<textarea>").addClass("form-input w-100")
						.attr("placeholder","변경 내용 작성").val(replyContent);

		var confirmButton = $("<button>").addClass("form-btn bosung ms-10").text("수정")
								.click(function(){
									...
									$.ajax({
										...
										data:{
											...
											reviewReplyContent:textarea.val()
										},
										success:function(response){
											loadList();
										},
